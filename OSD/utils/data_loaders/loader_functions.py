...
def load_patients(self,pat_list,start_label,batch_size=64):
    ...
    while success<len(label_start):
        ...
        try:
            pat = '/Users/erik-janmeulenbrugge/Documents/EJ_REPOS/OSD_repo/OSD_repo/OSD_data/Pre-Processed/sub-S0001111189075_ses-1/sub-S0001111189075_ses-1.h5'
            with h5.File(pat,'r') as f:
                ...
                #check for nan
                if np.any(np.isnan(f['annotations']['stage_expert_0'][start_idx:start_idx+600*self.batch_size])):
                    i+=1 
                    continue
